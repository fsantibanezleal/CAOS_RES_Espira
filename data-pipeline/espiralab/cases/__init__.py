...
CASES: dict[str, Case] = {
    ...
    "thermal-success-rate": Case(
        ...
        "Measured 2026-09-22 on the corrected engine with 600 copies per point: 0.735 against 0.985 at a "
        "stability factor of one, 0.810 against 1.000 at two, and 0.983 against 1.000 at ten. (Until "
        "spinoct 0.18.000 the field was applied with the wrong sign and this text quoted 0.952 at two, "
        "a field that destabilized the path.) The declared window (10 to 80) was corrected to (1 to 20) from "
    ...
    "continuum-cross-check": Case(
        ...
        code="C22",
        title="Continuum cross-check: the lattice wall barrier against the closed form",
        category="D. Beyond the macrospin",
        reason="The free-chain results rest on the minimum-energy-path barrier of a discrete chain. In the "
        "continuum limit that barrier must become the energy of a Bloch wall, 2 sqrt(2 J K) for a chain "
        "with exchange J and anisotropy K per site (the continuum wall energy 4 sqrt(A K) written in "
        "lattice units), and the lattice must approach it from below as the wall spans more sites. The "
        "plan asked for agreement with an established micromagnetic code; a closed-form limit is a "
        "stronger reference than a second code, because it has no discretization of its own.",
        expectation="The barrier converges onto the continuum wall energy from below, and the deficit "
        "falls as one over the wall width squared, the leading discreteness correction. Measured "
        "2026-09-18 over one to twelve sites per wall width (J/K from 2 to 288, chains of twelve widths): "
        "0.9547, 0.9891, 0.9973, 0.9988, 0.9993 and 0.9997 of the continuum value, with the deficit times "
        "the width squared between 0.0425 and 0.0453 throughout. Before engine 0.15.000 the solver "
        "diverged above J/K of about 24 and still returned a barrier, 43 times the continuum value at "
        "J/K = 40; the sweep reaches far past that on purpose.",
        kill_criterion="A barrier above the continuum wall energy, an unconverged path reported as a "
        "barrier, or a deficit that does not shrink like the width squared: any of them means the lattice "
        "energy, the string method or the continuum limit is wrong.",
        ...
        primary_method="R16",
        observable=Observable(
            key="barrier_over_continuum",
            label="Barrier over the continuum wall energy",
            unit="ratio",
            is_field_cost=False,
            note="The case checks an energy barrier against a closed form, so it reports the ratio of the "
            "lattice barrier to 2 sqrt(2 J K). No pulse is involved and no switching cost is reported.",
        ),
        methods=("R16",),
        sources=("10.1063/1.2720838", "10.1016/j.cpc.2015.07.001"),
    ),
