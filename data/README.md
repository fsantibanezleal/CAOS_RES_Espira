...
| Path | What | Git |
|---|---|---|
| `artifacts/` | The canonical, committed results the web app replays: `index.json`, one `<case>.json` per case, `novel.json` (the reliability front and the two-mode lattice comparison), `lattice_ocp.json` (the free chain crossover map), `patch_ocp.json` (the two-dimensional patch sweep, cases C20 and C21), `live_parity.json` (the values the browser's own implementation of the live lane must reproduce), `pareto.json` (the four-objective device trade-off per material, rung R14) | committed |
