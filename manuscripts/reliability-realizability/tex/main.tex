...
\newcommand{\docversion}{v4}
\newcommand{\docdate}{22 September 2026}
\newcommand{\doclicense}{CC BY 4.0}
\newcommand{\versiondoi}{10.5281/zenodo.22902338}
...
distribution, we reproduce the published stabilization physics for CrSBr, the half-hyperbolic bare path
that a field of order the anisotropy field removes, and we add the missing axis: the added switching
cost, which grows quadratically with the field. The price is not small: a field of one anisotropy field
already costs 2.5 times the bare optimal switching cost, and two anisotropy fields cost ten times it.
Nor is the benefit universal. At a thermal-stability factor of twenty the bare optimal pulse already
reverses every one of 400 copies, so the field buys nothing and costs a great deal; the reliability gain
appears only below a stability factor of about ten, where at a factor of one it lifts the success rate
from 0.735 to 0.985 and removes 94 per cent of the failures. The deterministic hyperbolicity integral,
which needs no ensemble, predicts that gain in every row of the grid that can distinguish its ends. The
front makes the reliability-versus-energy trade explicit and quantitative, and it is reproducible from
committed artifacts and a live instance.
\end{abstract}
...
0.5 & 0.33 & $1.000 \pm 0.005$ & $6.1\times10^{-12}$ \\
1.0 & 0.00 & $1.000 \pm 0.005$ & $2.5\times10^{-11}$ \\
1.5 & 0.00 & $1.000 \pm 0.005$ & $5.5\times10^{-11}$ \\
...
predicts~\citep{badarneh2023stability}. Second, the success rate is unity at every field in the sweep,
including at $B_r = 0$: at this stability factor the thermal kicks are too weak, over ten Larmor times,
to exploit the instability the analysis reports, so the front here measures a price and no gain. Third,
and new here, the added cost grows quadratically with the field. Measured against
...
\midrule
1 & 1.74 & $0.735 \pm 0.035$ & $0.985 \pm 0.010$ \\
2 & 0.87 & $0.810 \pm 0.031$ & $1.000 \pm 0.003$ \\
3 & 0.58 & $0.870 \pm 0.027$ & $1.000 \pm 0.003$ \\
5 & 0.35 & $0.932 \pm 0.020$ & $1.000 \pm 0.003$ \\
...
The longitudinal field raises the success rate wherever the bare pulse is fragile: from 0.735 to 0.985
at $\Delta = 1$, which removes 94 per cent of the failures, and to unity within the interval from
$\Delta = 2$ upward. Above $\Delta \approx 10$ the bare pulse already succeeds essentially always and
the gain falls inside the confidence interval. At $\Delta = 20$ this independent run, with more copies
and a finer step, gives $0.998 \pm 0.003$ for the bare pulse, consistent with the $1.000 \pm 0.005$ of
Table~\ref{tab:front}.

The two tables also test a claim that is cheaper than either of them. The hyperbolicity integral along
the path, $\int \max(0, -w_1 w_2)\,dt$, is deterministic and comes from the same Hessian the
optimal-control solver already has, so it predicts reliability without an ensemble at all. Over a grid
of five fields and six stability factors, 1{,}000 copies per cell, five rows separate their ends by more
than both confidence intervals, and in all five the measured failure rate falls with the penalty. The
sixth, at $\Delta = 20$, has nothing to predict: neither end fails.
